...
### Examples
```jldoctest; setup=:(using Aesop)
julia> polar = Aesop.load_polar("case9");

julia> powerflow = Aesop.PowerFlowBalance(polar) ∘ Aesop.PolarBasis(polar);

julia> jx = Aesop.Jacobian(polar, powerflow, State());

julia> stack = Aesop.NetworkStack(polar);

julia> conv = Aesop.nlsolve!(NewtonRaphson(verbose=1), jx, stack);
#it 0: 2.64764e+00
#it 1: 2.03366e-01
#it 2: 2.94166e-03
#it 3: 8.85300e-07
